...
merged_data <- final_data %>%
  left_join(data_2012, by = c("COUNTY" = "FIPS",  # Merge by COUNTY (FIPS code)
                              "zip_ldss" = "ZIP_VDSS_clean",  # Merge by cleaned LDSS ZIP
...
# Find rows where "dist" is not NA but "HAVERSINE_DISTANCE_METERS" is NA
missing_haversine <- merged_data %>%
  filter(!is.na(dist) & is.na(HAVERSINE_DISTANCE_METERS))
...
output_path <- "C:/Users/dscra/OneDrive - University of Virginia/VA_CommonHelp/Data Outputs/VDSS_Final_Data/final_data_merged.xlsx"
