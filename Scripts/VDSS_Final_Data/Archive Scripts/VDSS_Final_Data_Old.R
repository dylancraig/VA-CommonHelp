...
# Merge with the main dataset
data <- data %>%
  left_join(zip_treat, by = "ZIP")

# --- Step 7: Preparing the Final Dataset ---
# Filter for unique combinations and select necessary columns
data <- data %>%
  distinct(ZIP, COUNTY, MONTH_YEAR, RES_RATIO, BUS_RATIO, OTH_RATIO, TOT_RATIO,
           USPS_ZIP_PREF_CITY, USPS_ZIP_PREF_STATE, TYPE, RESIDENTIAL, ZCTA,
           bad_zipcounty, N, ldss_fips, zip_ldss, dist, treat_zip, 
           treat_g50, treat_g75, treat_l50, treat_l75)
